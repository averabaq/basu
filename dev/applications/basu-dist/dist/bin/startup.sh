...
. "/opt/cbi4api/basu/conf/setenv.sh"

APP_NAME="basu-server"
export JAVA_OPTS="-server -Xms512m -Xmx1024m -XX:+AggressiveHeap -XX:MaxPermSize=512m -noverify -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=5050 -Dcom.sun.management.jmxremote.port=21113 -Dcom.sun.management.jmxremote.authenticate=false -Dcom.sun.management.jmxremote.ssl=false -Dconfig=file://$BASU_HOME/etc/basu.properties -Dspring.profiles.active=default"
export HBASE_HOME="$BASU_HOME/apps/hbase-${org.apache.hbase.version}"
export HBASE_CONF_DIR="$HBASE_HOME/conf"
export HBASE_LOG_DIR="$HBASE_HOME/logs"
export HBASE_IDENT_STRING="basu"
export HBASE_PID_DIR="$BASU_HOME/var/hbase"
export HBASE_PID="$HBASE_PID_DIR/hbase-$HBASE_IDENT_STRING-master.pid"
export CATALINA_HOME="$BASU_HOME/apps/apache-tomcat-${org.apache.tomcat.version}"
export CATALINA_BASE="$BASU_HOME/apps/apache-tomcat-${org.apache.tomcat.version}"
export CATALINA_PID="$BASU_HOME/var/tomcat/$APP_NAME-tomcat.pid"
export CATALINA_OPTS="-Djava.security.egd=file:/dev/urandom -Djmagick.systemclassloader=no "
export LANG=en_GB.UTF-8
HBASE_GRACEPERIOD=20
TOMCAT_GRACEPERIOD=10
STARTUP_WAIT=30
...
echo "$APP_NAME process $PID: FAILED to start (see $BASU_HOME/logs/catalina.out)"
exit 1
