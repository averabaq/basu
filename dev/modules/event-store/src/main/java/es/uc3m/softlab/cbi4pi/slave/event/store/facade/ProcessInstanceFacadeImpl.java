...
import es.uc3m.softlab.cbi4pi.slave.event.store.dao.ProcessInstanceDAO;
import es.uc3m.softlab.cbi4pi.slave.event.store.dao.ProcessMappingDAO;
...
import es.uc3m.softlab.cbi4pi.slave.event.store.entity.ProcessModelMapping;
...
/**
 * Facade design pattern class implementation for the {@link es.uc3m.softlab.cbi4pi.slave.event.store.entity.ProcessInstance} 
...
public class ProcessInstanceFacadeImpl implements ProcessInstanceFacade {
    ...
    public ProcessInstance getProcessInstance(String processId, Source source) throws ProcessInstanceException {
		...
		ProcessInstance instance = processInstanceDAO.findBySourceData(processId, source);
		if (instance == null) {
			logger.debug("Cannot get process instance. Process instance with source data as pairs of (" + processId + ", " + source + ") does not exist.");
		}
		logger.debug("Process instance " + instance + " retrieved successfully.");
		return instance;
	}  
	...
	public void saveProcessInstance(ProcessInstance instance) throws ProcessInstanceException {
		...
		ProcessInstance _instance = processInstanceDAO.findBySourceData(instance.getInstanceSrcId(), instance.getModel().getSource());
		if (_instance != null) {
