...
public class ETLEvent implements MessageListener {
	...
	public void onMessage(Message msg) {
		try {
			es.uc3m.softlab.cbi4pi.slave.event.subscriber.xsd.bpaf.extension.Event bpafEvent;			
			/* extract event */
			bpafEvent = extractor.extractEvent(msg);
			/* transform event */
			es.uc3m.softlab.cbi4pi.slave.event.store.entity.Event event = transformer.transform(bpafEvent);
