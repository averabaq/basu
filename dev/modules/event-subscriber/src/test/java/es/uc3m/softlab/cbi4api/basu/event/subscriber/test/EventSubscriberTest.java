...
import es.uc3m.softlab.cbi4api.basu.event.subscriber.ETLProcessor;
import es.uc3m.softlab.cbi4api.basu.event.subscriber.EventConverter;
...
public class EventSubscriberTest extends AbstractShowcaseTest  {
    ...
    @EndpointInject(ref = "basu.bpaf.queue.endpoint")
    ProducerTemplate basuProducer; 	
    ...
    public void testETLTransformer() throws Throwable { 		
		...
		for (int i = 0; i < 50; i++) {
			es.uc3m.softlab.cbi4api.basu.event.store.domain.Event _event = null;
			Event event = generateMockEvent();		
			try {
