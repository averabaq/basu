...
public class Main {
	...
	public static void main(String[] args) {
		try {
			...
			if (args.length > 0) {
				...
				// command line
				StringBuffer bpeql = new StringBuffer();
				for (int i = s; i < args.length; i++) {
					bpeql.append(args[i]);					
					//parse(new File(args[i]));
				}
				System.out.println("BPEQL query: " + bpeql.toString());
				parse(bpeql.toString());
			} else {
				System.err.println("Usage: java -jar yourproject-yourversion-jar-with-dependencies.jar <directory | filename.dmo>");
			}
		} catch (Exception ex) {
	...
	public static void parse(String source) throws Exception {
		...
		try {
			...
			// specification for the tree, which will allow the users to
			// visualize
			...
			try {
				if (parser.getNumberOfSyntaxErrors() == 0) {
					...
					walker.stmt();		
					System.out.println("HiveQL: [ " + walker.getHiveQL() + " ]");
			...
			if (makeDot && tokens.size() < 4096) {
				...
				Process proc = Runtime.getRuntime().exec(
						"dot -Tpng -o" + source + "png " + outputName);
				proc.waitFor();
				stop = System.currentTimeMillis();
		...
		try {
			...
			// Else find out if it is an ASP.Net file and parse it if it is
				...
				if (sourceFile.length() > 3) {
					String suffix = sourceFile.substring(
							sourceFile.length() - 4).toLowerCase();
