...
import es.uc3m.softlab.cbi4api.basu.event.store.mapred.MapReduceInnerCorrelation;
...
public class ProcessInstanceDAOImpl implements ProcessInstanceDAO {
    ...
    public ProcessInstance findByCorrelationData(Model model, Set<EventCorrelation> correlation) throws IllegalArgumentException {
		...
		try {
			...
			long end = System.nanoTime();
			stats.writeStat(Stats.Operation.MAP_REDUCE_PROCESS_CORRELATION, MapReduceInnerCorrelation.class, _model, ini, end);
		} catch (Exception ex) {
