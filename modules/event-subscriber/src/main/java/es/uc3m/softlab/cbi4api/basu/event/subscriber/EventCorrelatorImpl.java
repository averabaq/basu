		...
		if (event.isSetActivityInstanceID()) {
			...
			if (activityInstance == null) {				
				...
				if (event.isSetActivityParent()) {
                    Model activityParentModel = (Model)modelFacade.getModel(event.getActivityParent().getActivityDefinitionID(), source, ModelType.ACTIVITY);
					ActivityInstance parent = activityInstanceFacade.getActivityInstance(event.getActivityParent().getActivityInstanceID(), activityParentModel);
